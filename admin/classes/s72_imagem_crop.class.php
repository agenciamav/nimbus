...
// funções para redimensionamento
	...
	*/	
	...
	public function redimensiona( $nova_largura = 0, $nova_altura = 0, $tipo = '' ) 
	{
		...
		switch( $tipo )
		{
			case 'crop': 				// diminui no tamanho máximo possivel e recorta do recorte o resto
				$this->resizeCrop();
				break;
			case 'fill':				// diminui no tamanho especificado e o resto da imagem fica com fundo
				$this->resizeFill();
				break;
			default:					// nesse caso diminui no tamanho especificado e espicha a img
				$this->resize();
				break;
		}
	 ...
	 * redimensionar()
	 *
	 * @description	Redimensiona uma imagem, desprezando proporções
	 * @param		largura		Largura que será utilizada na imagem final
	 * @param		altura		Altura que será utilizada na imagem final
	 */
	public function redimensionar($largura, $altura) {
		$img = imagecreatetruecolor($largura, $altura);
		imagecopyresampled($img, $this->img, 0, 0, 0, 0, $largura, $altura, $this->largura, $this->altura);
		$this->img = $img;
	 ...
	 * proporcional()
	 *
	 * @description	Redimensiona uma imagem mantendo as proporções dela obedecendo a um tamanho máximo.
	 * @param		dim		Dimensão que será utilizada para redimensionar a foto
	 * @param		sentido	Sentido que será levado em consideração para o redimensionamento (x, y, auto)
	 * @param		esticar	Booleano indicando se a imagem deve ser esticada caso seja menor do que as proporções
	 * @return		LARGURAxALTURA
	*/
	public function proporcional($dim, $sentido = 'auto', $esticar = false) {
		if ($sentido == 'auto')
			$sentido = ($this->largura > $this->altura) ? 'x' : 'y';
		
		if ($sentido == 'x') {
			if (($this->largura < $dim && $esticar) || $this->largura > $dim) {
				$largura = $dim;
				$fator = imagesx($this->img) / $dim;
				$altura = imagesy($this->img) / $fator;
			} else {
				$largura = $altura = false;
			}
		} else {
			if (($this->altura < $dim && $esticar) || $this->altura > $dim) {
				$altura = $dim;
				$fator = imagesy($this->img) / $dim;
				$largura = imagesx($this->img) / $fator;
			} else {
				$largura = $altura = false;
			}
		}
	 ...
	 * biProporcional()
	 *
	 * @description	Redimensiona uma imagem mantendo as proporções dela e obedecendo a um tamanho máximo de largura e altura.
	 * @param		largura		Largura máxima
	 * @param		altura		Altura máxima
	 */
	public function biProporcional($largura, $altura) 
	{
		if ($this->largura > $largura && $this->altura > $altura) {
			$this->proporcional($largura, 'x');
			if ($this->altura > $altura)
				$this->proporcional($altura, 'y');
		} elseif ($this->largura > $largura) {
			$this->proporcional($largura, 'x');
		} elseif ($this->altura > $altura) {
			$this->proporcional($altura, 'y');
		}
		//echo '<br>w:'.$this->largura.'<br>h:'.$this->altura.'<br>largura:'.$largura.'<br>altura:'.$altura;
		return true;
	}
